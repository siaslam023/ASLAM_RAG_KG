...
with st.sidebar:
    ...
    # 2. Neo4j Credentials Handling (Loads securely from .env on VPS)
    env_neo4j_uri = os.getenv("NEO4J_URI", "neo4j+s://eb0b7703.databases.neo4j.io").strip()
    env_neo4j_user = os.getenv("NEO4J_USERNAME", "neo4j").strip()
    env_neo4j_password = os.getenv("NEO4J_PASSWORD", "").strip()

    neo4j_uri_input = st.text_input(
        "Neo4j URI:",
        value="",
        placeholder="neo4j+s://eb0b7703.databases.neo4j.io",
        help="Leave blank to default to server .env variable."
    )
    neo4j_user_input = st.text_input(
        "Neo4j Username:",
        value="",
        placeholder=env_neo4j_user if env_neo4j_user else "neo4j",
        help="Leave blank to default to server .env variable."
    )
    ...
    # Resolve credentials and sanitize URI
    ...
    # ✅ FIX: Automatically convert neo4j+ssc:// to neo4j+s:// for AuraDB
    if raw_uri.startswith("neo4j+ssc://"):
        raw_uri = raw_uri.replace("neo4j+ssc://", "neo4j+s://")
